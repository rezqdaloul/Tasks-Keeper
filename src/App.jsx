...
const THEMES = {
  light: {
    name:"Light", emoji:"☀️",
    systemBg:"#F2F2F7", card:"rgba(255,255,255,0.90)", cardAlt:"rgba(242,242,247,0.92)", cardAlt2:"#E5E5EA",
    ...
    rRedBg:"rgba(255,59,48,0.07)", rRedBd:"#FF3B30", rRedTx:"#C8001A", rRedDt:"#FF3B30",
    rOrgBg:"rgba(255,149,0,0.07)", rOrgBd:"#FF9500", rOrgTx:"#7D3F00", rOrgDt:"#FF9500",
    star:"#FF9500", pinBg:"rgba(255,149,0,0.05)",
    tabBg:"rgba(255,255,255,0.85)", tabBorder:"rgba(0,0,0,0.06)",
    cardBorder:"rgba(0,0,0,0.06)", cardShadow:"0 2px 20px rgba(0,0,0,0.07)", blur:"blur(20px) saturate(180%)", glow:"rgba(0,122,255,0.22)",
  },
  dark: {
    name:"Dark", emoji:"🌙",
    systemBg:"#000000", card:"rgba(28,28,30,0.92)", cardAlt:"rgba(44,44,46,0.88)", cardAlt2:"rgba(58,58,60,0.85)",
    sep:"rgba(255,255,255,0.07)", sepHard:"rgba(255,255,255,0.18)",
    text:"#FFFFFF", muted:"#8E8E93", hint:"#3A3A3C",
    primary:"#0A84FF", primaryDim:"rgba(10,132,255,0.14)",
    success:"#30D158", danger:"#FF453A", warn:"#FF9F0A",
    rRedBg:"rgba(255,69,58,0.12)", rRedBd:"#FF453A", rRedTx:"#FF9999", rRedDt:"#FF453A",
    rOrgBg:"rgba(255,159,10,0.12)", rOrgBd:"#FF9F0A", rOrgTx:"#FFB347", rOrgDt:"#FF9F0A",
    star:"#FF9F0A", pinBg:"rgba(255,159,10,0.07)",
    tabBg:"rgba(28,28,30,0.88)", tabBorder:"rgba(255,255,255,0.08)",
    cardBorder:"rgba(255,255,255,0.07)", cardShadow:"0 4px 24px rgba(0,0,0,0.5)", blur:"blur(24px) saturate(200%)", glow:"rgba(10,132,255,0.28)",
  },
  midnight: {
    name:"Midnight", emoji:"🌌",
    systemBg:"#06060F", card:"rgba(12,12,28,0.92)", cardAlt:"rgba(18,18,38,0.88)", cardAlt2:"rgba(26,26,52,0.85)",
    sep:"rgba(129,140,248,0.10)", sepHard:"rgba(129,140,248,0.22)",
    text:"#E8EAFF", muted:"#7B82B8", hint:"#3D4275",
    primary:"#818CF8", primaryDim:"rgba(129,140,248,0.14)",
    success:"#34D399", danger:"#F87171", warn:"#FBBF24",
    rRedBg:"rgba(248,113,113,0.10)", rRedBd:"#F87171", rRedTx:"#FCA5A5", rRedDt:"#F87171",
    rOrgBg:"rgba(251,191,36,0.10)", rOrgBd:"#FBBF24", rOrgTx:"#FDE68A", rOrgDt:"#FBBF24",
    star:"#FBBF24", pinBg:"rgba(129,140,248,0.07)",
    tabBg:"rgba(12,12,28,0.90)", tabBorder:"rgba(129,140,248,0.15)",
    cardBorder:"rgba(129,140,248,0.10)", cardShadow:"0 4px 32px rgba(80,60,200,0.15)", blur:"blur(24px) saturate(200%)", glow:"rgba(129,140,248,0.35)",
  },
  sand: {
    name:"Sand", emoji:"🏜️",
    systemBg:"#FAF7F2", card:"rgba(255,255,255,0.88)", cardAlt:"rgba(245,240,232,0.90)", cardAlt2:"rgba(237,229,216,0.88)",
    sep:"rgba(160,120,80,0.11)", sepHard:"rgba(160,120,80,0.28)",
    text:"#2C1A0E", muted:"#8C6A4A", hint:"#C4A882",
    primary:"#C1440E", primaryDim:"rgba(193,68,14,0.09)",
    success:"#3D8B37", danger:"#C1440E", warn:"#D4890A",
    rRedBg:"rgba(193,68,14,0.07)", rRedBd:"#C1440E", rRedTx:"#8B2000", rRedDt:"#C1440E",
    rOrgBg:"rgba(212,137,10,0.07)", rOrgBd:"#D4890A", rOrgTx:"#7D4E00", rOrgDt:"#D4890A",
    star:"#D4890A", pinBg:"rgba(212,137,10,0.05)",
    tabBg:"rgba(255,255,255,0.90)", tabBorder:"rgba(160,120,80,0.14)",
    cardBorder:"rgba(160,120,80,0.09)", cardShadow:"0 2px 16px rgba(160,120,80,0.09)", blur:"blur(20px) saturate(160%)", glow:"rgba(193,68,14,0.22)",
  },
  forest: {
    name:"Forest", emoji:"🌲",
    systemBg:"#0A1A0C", card:"rgba(16,28,18,0.92)", cardAlt:"rgba(24,42,26,0.88)", cardAlt2:"rgba(32,56,36,0.85)",
    sep:"rgba(111,207,151,0.10)", sepHard:"rgba(111,207,151,0.22)",
    text:"#D4EDDA", muted:"#6B9E78", hint:"#2E5035",
    primary:"#6FCF97", primaryDim:"rgba(111,207,151,0.13)",
    success:"#6FCF97", danger:"#FF6B6B", warn:"#FFD93D",
    rRedBg:"rgba(255,107,107,0.10)", rRedBd:"#FF6B6B", rRedTx:"#FFA0A0", rRedDt:"#FF6B6B",
    rOrgBg:"rgba(255,217,61,0.10)", rOrgBd:"#FFD93D", rOrgTx:"#FFE88A", rOrgDt:"#FFD93D",
    star:"#FFD93D", pinBg:"rgba(111,207,151,0.07)",
    tabBg:"rgba(16,28,18,0.90)", tabBorder:"rgba(111,207,151,0.12)",
    cardBorder:"rgba(111,207,151,0.09)", cardShadow:"0 4px 24px rgba(0,0,0,0.4)", blur:"blur(24px) saturate(200%)", glow:"rgba(111,207,151,0.28)",
  },
  slate: {
    name:"Slate", emoji:"🪨",
    systemBg:"#1A2030", card:"rgba(28,36,50,0.92)", cardAlt:"rgba(36,46,64,0.88)", cardAlt2:"rgba(46,58,80,0.85)",
    sep:"rgba(246,173,85,0.10)", sepHard:"rgba(246,173,85,0.22)",
    text:"#EDF2F7", muted:"#A0AEC0", hint:"#4A5568",
    primary:"#F6AD55", primaryDim:"rgba(246,173,85,0.13)",
    success:"#68D391", danger:"#FC8181", warn:"#F6AD55",
    rRedBg:"rgba(252,129,129,0.10)", rRedBd:"#FC8181", rRedTx:"#FEB2B2", rRedDt:"#FC8181",
    rOrgBg:"rgba(246,173,85,0.10)", rOrgBd:"#F6AD55", rOrgTx:"#FCD34D", rOrgDt:"#F6AD55",
    star:"#F6AD55", pinBg:"rgba(246,173,85,0.07)",
    tabBg:"rgba(28,36,50,0.90)", tabBorder:"rgba(246,173,85,0.12)",
    cardBorder:"rgba(246,173,85,0.09)", cardShadow:"0 4px 24px rgba(0,0,0,0.4)", blur:"blur(24px) saturate(200%)", glow:"rgba(246,173,85,0.28)",
  },
};
...
export default function App() {
  ...
  const [users,        setUsers]       = useState(()=>{
    try{const r=localStorage.getItem("dt_v5_users");if(r){const p=JSON.parse(r);if(p&&typeof p==="object"&&Object.keys(p).length>0)return p;}
    }catch(e){}return DEFAULT_DATA;
  });
  const [curUser,      setCurUser]     = useState(null);
  const [curTopic,     setCurTopic]    = useState(null);
  const [themeName,    setThemeName]   = useState(() => {
    try { const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}"); return p.themeName||"dark"; } catch(e) { return "dark"; }
  });
  const [accentColor,  setAccentColor] = useState(() => {
    try { const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}"); return p.accentColor||""; } catch(e) { return ""; }
  });
  ...
  const [showBoarding, setShowBoarding]= useState(() => {
    try { const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}"); return p.showBoarding!==false; } catch(e){ return true; }
  });
  const [boardStep,    setBoardStep]   = useState(0);
  const [notifOn,      setNotifOn]     = useState(() => {
    try { const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}"); return p.notifOn===true; } catch(e){ return false; }
  });
  ...
  const _baseT = THEMES[themeName] || THEMES.dark;
  const T = accentColor ? { ..._baseT, primary:accentColor, primaryDim:accentColor+"22", glow:accentColor+"44" } : _baseT;
  ...
  const push = (next) => {
    ...
    try{localStorage.setItem("dt_v5_users",JSON.stringify(next));}catch(e){}
  ...
  useEffect(()=>{
    ...
    el.textContent = `
      ...
      *{-webkit-tap-highlight-color:transparent;}
      @keyframes dtComplete{0%{opacity:1;}20%{background:rgba(52,199,89,0.12);}55%{transform:scaleX(0.97) scaleY(0.97);opacity:0.5;max-height:80px;}100%{transform:scaleX(0.97) scaleY(0);opacity:0;max-height:0;padding-top:0;padding-bottom:0;overflow:hidden;}}
      @keyframes dtDelete{0%{transform:translateX(0) scaleY(1);opacity:1;}45%{transform:translateX(50%) scaleY(1);}100%{transform:translateX(110%) scaleY(0);opacity:0;max-height:0;padding:0;overflow:hidden;}}
      @keyframes dtFadeIn{from{opacity:0;transform:translateY(-6px);}to{opacity:1;transform:translateY(0);}}
      @keyframes rowEnter{from{opacity:0;transform:translateY(-8px) scaleY(0.95);}to{opacity:1;transform:translateY(0) scaleY(1);}}
      @keyframes dtSlideInRight{from{transform:translateX(36px);opacity:0;}to{transform:translateX(0);opacity:1;}}
      @keyframes dtSlideInLeft{from{transform:translateX(-36px);opacity:0;}to{transform:translateX(0);opacity:1;}}
      ...
      @keyframes dtFabPop{0%{transform:scale(0.5);opacity:0;}70%{transform:scale(1.1);}100%{transform:scale(1);opacity:1;}}
      @keyframes dtPulse{0%,100%{opacity:1;}50%{opacity:0.65;}}
  ...
  useEffect(()=>{
    try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.showBoarding=showBoarding;p.notifOn=notifOn;p.themeName=themeName;p.accentColor=accentColor;localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e){}
  },[showBoarding,notifOn,themeName,accentColor]);
  ...
  const doExport=()=>{
    try{
      ...
      const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="daily-tasks-backup-"+today+".json";
      document.body.appendChild(a);a.click();setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url);},500);
      setExportMsg("success");setTimeout(()=>setExportMsg(""),3000);
    }catch(e){setExportMsg("error");setTimeout(()=>setExportMsg(""),3000);}
  };
  const doImport=()=>{ setImportMsg(""); importFileRef.current&&importFileRef.current.click(); };
  const handleImportFile=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        const importedUsers=parsed.data||parsed;
        if(!importedUsers||typeof importedUsers!=="object"||Object.keys(importedUsers).length===0){setImportMsg("error:Invalid backup file.");return;}
        const existingNames=Object.values(users).map(u=>u.name.toLowerCase().trim());
        const conflicts=Object.values(importedUsers).map(u=>u.name).filter(name=>existingNames.includes(name.toLowerCase().trim()));
        if(conflicts.length>0){setImportDialog({conflicts,pendingUsers:importedUsers});}
        else{doImportMerge(importedUsers);}
      }catch(err){setImportMsg("error:Could not read backup file.");}
    };
    reader.readAsText(file);
  };
  const doImportMerge=(importedUsers)=>{
    const merged=JSON.parse(JSON.stringify(users));let added=0,overwritten=0;
    Object.values(importedUsers).forEach(iu=>{
      const existing=Object.entries(merged).find(([,u])=>u.name.toLowerCase().trim()===iu.name.toLowerCase().trim());
      if(existing){merged[existing[0]]={...iu,id:existing[0]};overwritten++;}else{const nid=iu.id||Date.now().toString();merged[nid]={...iu,id:nid};added++;}
    });
    push(merged);setImportDialog(null);
    const parts=[];if(added>0)parts.push(added+" profile"+(added>1?"s":"")+" added");if(overwritten>0)parts.push(overwritten+" overwritten");
  ...
  // ── Styles ─────────────────────────────────────────────────────────────────
  const S={
    inp:(e)=>({backgroundColor:T.cardAlt,border:`1px solid ${T.cardBorder||T.sep}`,color:T.text,borderRadius:14,padding:"13px 15px",fontSize:15,outline:"none",width:"100%",boxSizing:"border-box",backdropFilter:T.blur||"blur(12px)",...(e||{})}),
    primBtn:(e)=>({backgroundColor:T.primary,color:"#fff",border:"none",borderRadius:14,padding:"15px",fontSize:16,fontWeight:700,cursor:"pointer",width:"100%",boxShadow:`0 4px 20px ${T.glow||T.primary+"44"}`,transition:"transform .15s, box-shadow .15s",...(e||{})}),
    ghost:(e)=>({background:"none",border:"none",color:T.primary,fontSize:15,cursor:"pointer",fontWeight:500,...(e||{})}),
    pill:(active,e)=>({padding:"7px 16px",borderRadius:20,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all .2s",backgroundColor:active?T.primary:"transparent",color:active?"#fff":T.muted,boxShadow:active?`0 2px 12px ${T.glow||T.primary+"44"}`:"none",...(e||{})}),
    card:(e)=>({backgroundColor:T.card,borderRadius:18,overflow:"hidden",border:`1px solid ${T.cardBorder||T.sep}`,boxShadow:T.cardShadow||"0 2px 16px rgba(0,0,0,0.08)",backdropFilter:T.blur||"blur(16px)",...(e||{})}),
    row:(e)=>({display:"flex",alignItems:"center",padding:"14px 16px",gap:12,...(e||{})}),
    sep:()=>({height:"1px",backgroundColor:T.sep,margin:"0 16px"}),
    label:(color)=>({fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:0.9,color:color||T.muted,marginBottom:8,paddingLeft:2}),
  };
  ...
  const TabBar=()=>(
    <div style={{flexShrink:0,padding:"6px 14px 10px",display:"flex",alignItems:"center",gap:10}}>
      <div style={{flex:1,display:"flex",alignItems:"center",
        backgroundColor:T.tabBg, backdropFilter:T.blur||"blur(20px)", WebkitBackdropFilter:T.blur||"blur(20px)",
        borderRadius:40, padding:"4px 8px",
        boxShadow:isDark?"0 8px 32px rgba(0,0,0,0.6), 0 2px 8px rgba(0,0,0,0.4)":"0 4px 24px rgba(0,0,0,0.10), 0 1px 6px rgba(0,0,0,0.07)",
        ...
        {[{id:"home",icon:<Home size={20}/>,label:"Home"},{id:"today",icon:<Sunrise size={20}/>,label:"Today"},{id:"calendar",icon:<CalendarDays size={20}/>,label:"Calendar"},{id:"settings",icon:<Settings size={20}/>,label:"Settings"}].map(tab=>{
          ...
          return(
            <button key={tab.id} onClick={()=>handleTab(tab.id)}
              ...
              <span style={{color:active?T.primary:T.muted,transition:"color .2s, transform .2s",transform:active?"scale(1.1)":"scale(1)",display:"inline-flex"}}>{tab.icon}</span>
      ...
      {showFab&&(
        <button onClick={openAdd} style={{width:52,height:52,borderRadius:26,flexShrink:0,backgroundColor:T.primary,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 6px 24px ${T.glow||T.primary+"55"}`,animation:"dtFabPop 0.35s cubic-bezier(0.34,1.56,0.64,1)"}}>
  ...
  const renderOnboarding=()=>{
    const done=()=>{
      try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.showBoarding=false;localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e){}
  ...
  const renderSettings=()=>(
    <div style={{flex:1,overflowY:"auto",padding:"20px 16px"}}>
      ...
      <div style={{...S.card(),marginBottom:20,padding:"14px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {Object.entries(THEMES).map(([key,theme])=>{
            const isActive=themeName===key;
            return(<button key={key} onClick={()=>{setThemeName(key);try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.themeName=key;localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e){}}}
              style={{padding:"12px 0",borderRadius:14,cursor:"pointer",border:`2px solid ${isActive?T.primary:T.sep}`,backgroundColor:theme.systemBg,display:"flex",flexDirection:"column",alignItems:"center",gap:6,transition:"all .2s",boxShadow:isActive?`0 0 14px ${T.glow||T.primary+"44"}`:"none"}}>
              <span style={{fontSize:22}}>{theme.emoji}</span>
              <span style={{fontSize:11,fontWeight:700,color:isActive?T.primary:theme.muted}}>{theme.name}</span>
              <div style={{display:"flex",gap:3}}>{[theme.systemBg,theme.primary,theme.success].map((c,i)=>(<div key={i} style={{width:6,height:6,borderRadius:"50%",backgroundColor:c,border:`1px solid ${theme.sep}`}}/>))}</div>
            </button>);
          })}
        </div>
      </div>
      ...
      <div style={{...S.card(),marginBottom:20,padding:"14px 16px"}}>
        <div style={{fontSize:13,color:T.muted,marginBottom:10}}>Override the primary color across all themes</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:9,marginBottom:10}}>
          {["#007AFF","#FF3B30","#FF9500","#34C759","#AF52DE","#FF2D55","#00C7BE","#5856D6","#FF6B6B","#4ECDC4","#F7DC6F","#E74C3C"].map(c=>(
            <button key={c} onClick={()=>{setAccentColor(c);try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.accentColor=c;localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e){}}}
              style={{width:32,height:32,borderRadius:16,border:`3px solid ${accentColor===c?T.text:"transparent"}`,backgroundColor:c,cursor:"pointer",transition:"all .18s",boxShadow:accentColor===c?`0 0 10px ${c}88`:"none"}}/>
          ))}
          <label style={{width:32,height:32,borderRadius:16,border:`2px dashed ${T.sepHard}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,color:T.muted,overflow:"hidden",position:"relative"}}>
            <span style={{pointerEvents:"none"}}>+</span>
            <input type="color" value={accentColor||T.primary} onChange={e=>{setAccentColor(e.target.value);try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.accentColor=e.target.value;localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e2){}}} style={{position:"absolute",inset:0,opacity:0,cursor:"pointer",width:"100%",height:"100%"}}/>
          </label>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            <div style={{width:18,height:18,borderRadius:9,backgroundColor:accentColor||T.primary,border:`1px solid ${T.sep}`}}/>
            <span style={{fontSize:13,color:T.muted}}>{accentColor?accentColor.toUpperCase():"Theme default"}</span>
          </div>
          {accentColor&&<button onClick={()=>{setAccentColor("");try{const p=JSON.parse(localStorage.getItem("dt_v5_prefs")||"{}");p.accentColor="";localStorage.setItem("dt_v5_prefs",JSON.stringify(p));}catch(e){}}} style={{fontSize:13,color:T.danger,background:"none",border:"none",cursor:"pointer",fontWeight:600}}>Reset</button>}
        </div>
      </div>
      ...
      <div style={{...S.card(),marginBottom:32}}>
        ...
        {exportMsg==="success"&&<div style={{margin:"0 16px 10px",padding:"8px 12px",backgroundColor:T.success+"18",borderRadius:10,fontSize:13,color:T.success,fontWeight:600}}>\u2713 Backup downloaded</div>}
        {exportMsg==="error"&&<div style={{margin:"0 16px 10px",padding:"8px 12px",backgroundColor:T.danger+"18",borderRadius:10,fontSize:13,color:T.danger}}>Export failed</div>}
        <div style={S.sep()}/>
        <button onClick={doImport} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"14px 16px",background:"none",border:"none",cursor:"pointer"}}><span style={{fontSize:15,color:T.text}}>Import Backup</span><Upload size={16} color={T.primary}/></button>
        ...
        <div style={S.sep()}/>
        <button onClick={()=>setResetDialog(true)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"14px 16px",background:"none",border:"none",cursor:"pointer"}}><span style={{fontSize:15,color:T.danger}}>Reset All Data</span><Trash2 size={16} color={T.danger}/></button>
    ...
    <div style={{backgroundColor:T.systemBg,width:"100%",height:"100%",overflow:"hidden",display:"flex",flexDirection:"column",position:"relative",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif"}}>
      ...
      {importDialog&&(
        <div style={{position:"absolute",inset:0,zIndex:96,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
          <div onClick={()=>setImportDialog(null)} style={{position:"absolute",inset:0,backgroundColor:"rgba(0,0,0,0.55)",animation:"dtOverlayIn 0.2s ease",backdropFilter:"blur(4px)"}}/>
          <div style={{position:"relative",backgroundColor:T.card,borderRadius:"24px 24px 0 0",zIndex:1,animation:"dtSheetUp 0.32s cubic-bezier(0.34,1.56,0.64,1)",border:`1px solid ${T.cardBorder||T.sep}`,backdropFilter:T.blur||"blur(20px)"}}>
            <div style={{display:"flex",justifyContent:"center",padding:"12px 0 8px"}}><div style={{width:40,height:4,borderRadius:2,backgroundColor:T.hint}}/></div>
            <div style={{padding:"8px 20px 16px",borderBottom:`1px solid ${T.sep}`}}>
              ...
              <div style={{fontSize:14,color:T.muted,lineHeight:1.5}}>The following profiles already exist. Importing will overwrite their data. All other profiles will be added safely.</div>
            </div>
            <div style={{padding:"12px 20px",maxHeight:180,overflowY:"auto"}}>
              {importDialog.conflicts.map(name=>(
                <div key={name} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",backgroundColor:T.rRedBg,borderRadius:12,marginBottom:8,border:`1px solid ${T.rRedBd}30`}}>
            ...
            <div style={{padding:"8px 20px 32px",display:"flex",flexDirection:"column",gap:10}}>
              <button onClick={()=>doImportMerge(importDialog.pendingUsers)} style={{padding:"15px",borderRadius:14,backgroundColor:T.danger,border:"none",cursor:"pointer",fontSize:16,fontWeight:700,color:"#fff",boxShadow:`0 4px 16px ${T.danger}44`}}>Continue Import</button>
              <button onClick={()=>setImportDialog(null)} style={{padding:"15px",borderRadius:14,backgroundColor:T.cardAlt,border:"none",cursor:"pointer",fontSize:16,fontWeight:600,color:T.text}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {resetDialog&&(
        <div style={{position:"absolute",inset:0,zIndex:96,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
          <div onClick={()=>setResetDialog(false)} style={{position:"absolute",inset:0,backgroundColor:"rgba(0,0,0,0.55)",animation:"dtOverlayIn 0.2s ease",backdropFilter:"blur(4px)"}}/>
          <div style={{position:"relative",backgroundColor:T.card,borderRadius:"24px 24px 0 0",zIndex:1,animation:"dtSheetUp 0.32s cubic-bezier(0.34,1.56,0.64,1)",border:`1px solid ${T.cardBorder||T.sep}`,backdropFilter:T.blur||"blur(20px)"}}>
            <div style={{display:"flex",justifyContent:"center",padding:"12px 0 8px"}}><div style={{width:40,height:4,borderRadius:2,backgroundColor:T.hint}}/></div>
            ...
            <div style={{padding:"0 20px 32px",display:"flex",flexDirection:"column",gap:10}}>
              <button onClick={()=>{try{localStorage.clear();}catch(e){}window.location.reload();}} style={{padding:"15px",borderRadius:14,backgroundColor:T.danger,border:"none",cursor:"pointer",fontSize:16,fontWeight:700,color:"#fff",boxShadow:`0 4px 16px ${T.danger}44`}}>Delete Everything</button>
              <button onClick={()=>setResetDialog(false)} style={{padding:"15px",borderRadius:14,backgroundColor:T.cardAlt,border:"none",cursor:"pointer",fontSize:16,fontWeight:600,color:T.text}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
